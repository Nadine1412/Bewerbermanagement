...
public class RecruiterManager {
	...
	public Map<String, Boolean> expressionRecruiter(Recruiter recruiter) {
		...
		boolean resultSurname = Pattern.matches("^[A-Z][a-z]+$", recruiter.getUserSurname()); // Keine Zahlen im
																								// Nachnamen
		boolean resultPosition = Pattern.matches("^[A-Z][A-Z0-9a-z.\\/_%+-]+", recruiter.getPosition()); // Großer
																											// Anfangsbuchstabe
		boolean resultEmail = Pattern.matches("[A-Z0-9a-z._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,64}",
				recruiter.getEmail()); // Überprüfen der Email-Adresse
