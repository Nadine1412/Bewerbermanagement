...
public class UserProfileController {
		...
		//Aufruf der Methode ExpressionApplicant
						...
						map.get("password"))
				{
					int counter = userDao.changeApplicantProfile(applicant);
						...
						if (counter > 0) {
							mv.addObject("msg", "Profil-Änderung erfolgreich.");
							mv.addObject("applicant", applicant);
							mv.setViewName("showApplicantProfile");
						} else {
					...
					if(!map.get("name")) {
						mv.addObject("errorName", "Falsche Eingabe. Bitte mit einem Großbuchstaben beginnen.");
					}
					if(!map.get("surname")) {
						mv.addObject("errorSurname", "Falsche Eingabe. Bitte mit einem Großbuchstaben beginnen.");
					}
					if(!map.get("birthday")) {
					...
					if(!map.get("subject")) {
						mv.addObject("errorSubject", "Falsche Eingabe. Bitte mit einem Großbuchstaben beginnen.");
					}
					if(!map.get("specialization")) {
						mv.addObject("errorSpecialization", "Falsche Eingabe. Bitte mit einem Großbuchstaben beginnen.");
					} 
					if(!map.get("salary")) {
	...
	public ModelAndView changeRecruiterProfil(
				...
				map.get("position") && map.get("email") && map.get("password"))
		{
			...
			if(!map.get("position")) {
				mv.addObject("errorPosition", "Falsche Eingabe. Bitte mit einem Großbuchstaben beginnen.");
			}
			if(!map.get("email")) {
				mv.addObject("errorEmail", "Bitte eine richtige E-Mailadresse eingeben.");
			}
