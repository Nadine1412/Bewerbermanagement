...
public class UserDao {
				...
				if (statementApplicant != null && counterUser != 0) {
					int counterApplicant = jdbcTemplate.update(statementApplicant,
							new Object[] { applicant.getEntryDate(), applicant.getSubject(),
									applicant.getSpecialization(), applicant.getSalary(), applicant.getEmail() });
		...
		if (statement != null) {
			try {
				String resultName = jdbcTemplate.queryForObject(statement,
						new Object[] { user.getEmail(), user.getPassword() }, new RowMapper<String>() {
			...
			try {
				int counter = jdbcTemplate.update(statement,
						new Object[] { applicant.getUserName(), applicant.getUserSurname(), applicant.getEmail(),
								...
								applicant.getSpecialization(), applicant.getSalary(), applicant.getPassword(),
								applicant.getUserId() });
	...
	public int changeRecruiterProfile(Recruiter recruiter) {
		...
		if (statement != null) {
			try {
				int counter = jdbcTemplate.update(statement,
						new Object[] { recruiter.getUserName(), recruiter.getUserSurname(), recruiter.getEmail(),
								recruiter.getBirthday(), recruiter.getEnterprise(), recruiter.getPosition(),
								recruiter.getPassword(), recruiter.getUserId() });
